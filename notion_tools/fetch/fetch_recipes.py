"""Export de la base Recettes vers `recipes.json`."""
...
from app.config import DATA_DIR, models_scorecard_db_id_1
...
RECIPES_DB_ID = models_scorecard_db_id_1
...
def fetch_recipes() -> list[Dict]:
    normalized_id = normalize_id(RECIPES_DB_ID)
    if not normalized_id:
        raise RuntimeError("Aucun identifiant de base Recettes fourni.")

    client = get_client()
    schema = get_database_properties(client, normalized_id)

    name_key = resolve_property_name(client, normalized_id, ["Name", "Nom", "Title"])
    if not name_key:
    ...
    if not name_key:
        ...
        raise RuntimeError(
            "Impossible de trouver la colonne titre dans la base Recettes. "
...
def main() -> None:
    print("➡️  Lecture de la base Recettes…")
    items = fetch_recipes()
    OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT_PATH.write_text(json.dumps(items, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"   ✅ {len(items)} recette(s) enregistrée(s) dans {OUTPUT_PATH}")
